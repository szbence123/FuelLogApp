...
export const insertCar = (name: string, regNumber: string) => {
    return executeQuery<CarModel[]>(db, "INSERT INTO CAR (name, regNumber) VALUES (?, ?)", [name, regNumber])
};

export const removeCar = (id: number, refetch: Function) => {
    return executeQuery<CarModel[]>(db, 'DELETE FROM CAR WHERE id = ?', [id]).then(r=>refetch())
};
